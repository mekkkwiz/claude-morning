...
elif [ "$(echo "$JSON_OUTPUT" | jq -r '.is_error')" = "true" ] || [ "$STATUS" -ne 0 ]; then
  API_ERROR=$(echo "$JSON_OUTPUT" | jq -r '.api_error_status // .error.message // .error // .message // .result // "Claude returned an error without details"')
  echo "$TIMESTAMP ERROR $API_ERROR"
else
  COST=$(echo "$JSON_OUTPUT" | jq -r '.total_cost_usd')
