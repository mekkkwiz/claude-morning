...
- A cron job fires `claude -p "ping"` every 5 minutes using Haiku (minimal cost)
...
services:
  claude-morning:
    ...
    command: daemon
    environment:
      - CLAUDE_MORNING_CRON_SCHEDULE=*/5 * * * *
      - TZ=Asia/Bangkok
...
|------------------------------|-------------------|---------------------------------|
| `CLAUDE_MORNING_CRON_SCHEDULE`| `*/5 * * * *`  | Cron schedule (comma-separated for multiple, e.g. `*/5 * * * *` or `0 8 * * *,0 20 * * *`) |
| `TZ`                    | (none/UTC)        | Timezone (e.g. `Asia/Bangkok`)   |

## Examples

### Run every 5 minutes in Tokyo timezone
